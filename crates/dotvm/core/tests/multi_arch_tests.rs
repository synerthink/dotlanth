...
use dotvm_core::{
    ...
    instruction::{
        ...
        memory::{StoreInstruction, LoadInstruction},
    ...
    opcode::{
        ...
        architecture_opcodes::*,
    },
    vm::{
        multi_arch_executor::MultiArchExecutor,
        vm_factory::{VMFactory, SimpleVMFactory},
...
fn test_arithmetic_across_architectures() {
    ...
    for (a, b, op, expected) in test_cases {
        ...
        {
            let mut executor = MultiArchExecutor::<Arch32>::new(
                VmArchitecture::Arch32, 
                VmArchitecture::Arch32
            ).expect("Failed to create 32-bit executor");
            ...
            assert!((result - expected).abs() < f64::EPSILON, 
                "32-bit: {:?} failed: {} != {}", op, result, expected);
        }

        // Test on 64-bit architecture
        {
            let mut executor = MultiArchExecutor::<Arch64>::new(
                VmArchitecture::Arch64, 
                VmArchitecture::Arch64
            ).expect("Failed to create 64-bit executor");
            ...
            let result = executor.pop_operand().expect("No result on stack");
            assert!((result - expected).abs() < f64::EPSILON, 
                "64-bit: {:?} failed: {} != {}", op, result, expected);
        }
    }
}
    ...
    for (a, b, op, expected) in test_cases {
        // Test on 128-bit architecture
        {
            let mut executor = MultiArchExecutor::<Arch128>::new(
                ...
                VmArchitecture::Arch128
            ).expect("Failed to create 128-bit executor");
            ...
            let result = executor.pop_operand().expect("No result on stack");
            assert!((result - expected).abs() < f64::EPSILON, 
                "128-bit BigInt: {:?} failed: {} != {}", op, result, expected);
        }
        ...
        // Test on 256-bit architecture
        {
            let mut executor = MultiArchExecutor::<Arch256>::new(
                ...
                VmArchitecture::Arch256
            ).expect("Failed to create 256-bit executor");
            ...
            let result = executor.pop_operand().expect("No result on stack");
            assert!((result - expected).abs() < f64::EPSILON, 
                "256-bit BigInt: {:?} failed: {} != {}", op, result, expected);
        }
    }
}
...
fn test_memory_on_arch<A: Architecture + std::fmt::Debug>(arch: VmArchitecture) {
    let mut executor = MultiArchExecutor::<A>::new(arch, arch)
    ...
    assert!((result - expected).abs() < f64::EPSILON, 
        "{:?}: Memory load failed: {} != {}", arch, result, expected);
}
...
fn test_architecture_opcode_compatibility() {
    ...
    assert!(Arch512Opcodes::supports_opcode(&Opcode512::Base(Opcode256::Base(Opcode128::Base(Opcode64::Arithmetic(ArithmeticOpcode::Add))))));
    ...
    for &host_arch in &architectures {
        for &guest_arch in &architectures {
            ...
            if host_arch >= guest_arch {
                ...
                assert!(result.is_ok(), 
                    "Factory should create VM for host {:?} guest {:?}", 
...
fn test_performance_scaling() {
    ...
    for (arch, name) in arch_configs {
        ...
        match arch {
            VmArchitecture::Arch32 => {
                let mut executor = MultiArchExecutor::<Arch32>::new(arch, arch)
                    .expect("Failed to create executor");
                run_performance_test(&mut executor, iterations);
            }
            VmArchitecture::Arch64 => {
                let mut executor = MultiArchExecutor::<Arch64>::new(arch, arch)
                    .expect("Failed to create executor");
                run_performance_test(&mut executor, iterations);
            }
            VmArchitecture::Arch128 => {
                let mut executor = MultiArchExecutor::<Arch128>::new(arch, arch)
                    .expect("Failed to create executor");
                run_performance_test(&mut executor, iterations);
            }
            VmArchitecture::Arch256 => {
                let mut executor = MultiArchExecutor::<Arch256>::new(arch, arch)
                    .expect("Failed to create executor");
                run_performance_test(&mut executor, iterations);
            }
            VmArchitecture::Arch512 => {
                let mut executor = MultiArchExecutor::<Arch512>::new(arch, arch)
                    .expect("Failed to create executor");
                run_performance_test(&mut executor, iterations);
            }
        }
...
fn test_memory_scaling<A: Architecture + std::fmt::Debug>(arch: VmArchitecture, expected_bits: usize) {
    let _executor = MultiArchExecutor::<A>::new(arch, arch)
        .expect("Failed to create executor");
    
    // Verify architecture properties
    assert_eq!(A::WORD_SIZE * 8, expected_bits, 
        "Word size should match architecture bits");
